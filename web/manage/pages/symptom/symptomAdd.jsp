  ...
  <tbody>
    ...
    <tr>
      <td  colspan="3" valign="top" ><!-- form(input) 默认不显示，去掉display 样式后显示-->
        <fieldset>
        <legend>增加症状</legend>
            <form name="symptomAdd" method="post" action="${pageContext.request.contextPath}/symptomAction.do?method=addAction">
                <table  class="theform" >
                    <tr><th>症状所属位置：</th>
                        <td>
...
%>
                ...
                <tr><th>症状名称：</th>
                    <td><input type="text" name="symName" value="" /><input type="hidden" name="labelId" value="<%=labelId%>"/></td></tr>
                <tr><th>父症状：</th>
                    <td>
...
<script type="text/javascript">
    ...
    function checkInputValue() {
        var disName = document.getElementsByName("symName")[0].value;
        if(disName==null || disName==""){
            alert("症状名称不能为空");
            return;
        }
        document.getElementsByName("symptomAdd")[0].submit();
    }
</script>
                ...
                <tr><th>症状名称属性：</th>
                    <td>
                        <select name="symProperty">
                            <option value="1">是否</option>
                            <option value="2">文字描述</option>
                        </select></td></tr>

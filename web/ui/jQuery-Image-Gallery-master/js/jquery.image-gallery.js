...
(function (factory) {
    ...
    $.widget('blueimp.imagegallery', {
        ...
        _clickHandler: function (e) {
            ...
            if (e.pageX < (dialog.offset().left + 70)) {
                that._prev();
            } else if (e.pageX > (dialog.offset().left + dialog.width() - 70)){
                that._next();
            }else{
                //http://localhost:8086/bpps/attached/image/20130429/ 20130429231412_989_3_b.jpg
                var imgUrl = e.srcElement.src;
                var reg1 = /_[^_]*_/g;
                var reg2 = /_[^_]*_[^_]*_/g;
                reg1.test(imgUrl);
                reg2.test(imgUrl);
                var symId = imgUrl.substring(reg1.lastIndex, reg2.lastIndex-1);
                //alert(symId);
                
                var symDiseaseUrl = "/bpps/functionPage/symDiseaseSearch.jsp?symId="+symId;
                window.open(symDiseaseUrl, "symDisease");
            }
        },

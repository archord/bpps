...
/**
 * 该版本为多种症状求交集确定疾病，即同一个位置的疾病包含该位置的所有症状才搜索出该疾病。
 * 具体搜索方式：用户在症状检索页面选择部位，系统分页显示每个部位的症状列表，用户选择当前
 * 部位的症状后选择提交，系统根据选择的当前部位的症状，筛选出符合的疾病，并通过
 * hashmap<位置，疾病列表>存储在session中，后面每个部位依次这样处理。最后对所有疾病进行汇总，
 * 去除重复的疾病，并显示结果。
 * @author Administrator
 */
public class DiseaseAction1 extends DispatchAction {

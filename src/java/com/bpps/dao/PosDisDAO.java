...
public class PosDisDAO {
  ...
  public List<Integer> getPosIdByDisId(DatabaseManager dbm, int disId) {
  ...
  public List<Integer> getDisIdByPosId(DatabaseManager dbm, int posId) {
    log.debug("get posid by disid");
  ...
  public List<Disease> getDiseasesByPosId(int posId) {
    ...
    try {
      ...
      String sql = "select dis.dis_id, dis.dis_name, posdis.pos_dis_image_path dis_image_path "
              + "from disease dis, position_disease posdis "
              + "where posdis.pos_dis_flag = 1 and dis.dis_id=posdis.dis_id and posdis.pos_id=" + posId + " order by dis_id";
      DatabaseManager dbm = new DatabaseManager();
      ResultSet rs = dbm.doSelect(sql);
      while (rs.next()) {
  ...
  public List<Disease> getDiseasesByPosId(int posId, int typeId) {
    log.debug("get All Disease from position_disease");
    ...
    try {
      ...
      String sql = "select dis.dis_id, dis.dis_name, posdis.pos_dis_image_path dis_image_path "
              + "from disease dis "
              + "inner join position_disease posdis on posdis.pos_dis_flag = 1 and dis.dis_id=posdis.dis_id and posdis.pos_id=" + posId + " "
              + "where dis.dis_type=" + typeId + " order by dis_id";
      DatabaseManager dbm = new DatabaseManager();
      ResultSet rs = dbm.doSelect(sql);
      while (rs.next()) {
        Disease obj = new Disease();
  ...
  public void deletePosDisByDisId(int disId) {
    ...
    List<Integer> posIds = getPosIdByDisId(dbm, disId);
  ...
  public void updatePosDisStatus(DatabaseManager dbm, int posId, List<Integer> disChecked, List<Integer> disUnchecked) {
    ...
    if (!disUnchecked.isEmpty()) {
      ...
      System.out.println(disStr);
      String sql = "update position_disease set pos_dis_flag = 0 where pos_id = " + posId + " and dis_id in(" + disStr + ")";
      System.out.println(sql);
      dbm.doExecute(sql);
      String sql2 = "delete from symptom_disease where pos_id=" + posId + " and dis_id in(" + disStr + ")";
      dbm.doExecute(sql2);
    }
    if (!disChecked.isEmpty()) {
      String disStr = "";
      for (Integer disease : disChecked) {
        disStr += disease + ",";
      }
      disStr = disStr.substring(0, disStr.length() - 1);
      String sql = "update position_disease set pos_dis_flag = 1 where pos_id = " + posId + " and dis_id in(" + disStr + ")";
      dbm.doExecute(sql);
      for (Integer disease : disChecked) {
        //add
        symDisDAO.addSymDisByDisId(dbm, posId, disease);
      }
    }
//    for (Integer disease : disUnchecked) {
//      String sql = "update position_disease set pos_dis_flag = 0 where dis_id = ? and pos_id = " + posId;
//      Object[] objs = {0, disease};
//      dbm.doExecute(sql, objs);
//      //delete
//      symDisDAO.deleteSymDisByDisId(dbm, posId, disease);
//    }
//    for (Integer disease : disChecked) {
//      String sql = "update position_disease set pos_dis_flag = 1 where dis_id = ? and pos_id = " + posId;
